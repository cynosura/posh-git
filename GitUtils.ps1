...
function Get-GitTree {
	...
	$stat = Get-GitStatusInternal $gitDir $Global:GitPromptSettings ""
	
	if($stat){
		Get-Tree $stat.Working $stat.Index $gitDir
	} else {
		"fatal: Not a git repository (or any of the parent directories): .git" | Out-host
	}
}

function NullCoalesce([string]$a, [string]$b){
	if($a) {
		$a
	} else {
		$b
            ...
            if($_) {
                switch -regex ($_) {
                    '^(?<index>[^#])(?<working>.) (?<path1>.*?)(?: -> (?<path2>.*))?$' {
						$item = (NullCoalesce $matches['path2'] $matches['path1'])
